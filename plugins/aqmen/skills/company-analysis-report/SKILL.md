...
3. Read `references/company-analysis-content.md` — the single source of truth for what the deliverable covers, the module rules, and what to gather — then `references/company-analysis-structure.md` — the required section order
   and what belongs in each.
4. Start from `references/company-analysis-report-template.html` — the populated, fully-styled example for this module (it mirrors the deck's content, so report and deck show the same information). Replace its placeholder content, following the
